...
public class WorkController {
	...
	public ResponseEntity<List<WorkDTO>> getAllWorks(@RequestParam(required = false) String tag) {
		List<WorkDTO> works;
		if (tag != null && !tag.isEmpty()) {
			works = workService.getArtworksByTag(tag);
		} else {
			works = workService.getAllArtworks();
		}
		return ResponseEntity.ok(works);
	}
